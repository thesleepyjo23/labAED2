...
void AlocaTabelaPalavras ( char *ficheiro, st_texto *t)
{
  ...
  (*t).n_total_palavras = 0; /* (=) t->n_total_palavras=0; */
  (*t).n_dist_palavras = 0;
  f = AbreFicheiro ( ficheiro, "r" );
  /* "Definir a folha": altura - n_total_palavras e largura - n_max_caracteres */
...
void EscreveFicheiro ( char *ficheiro, st_texto *t )
{
  ...
  nome = (char*) malloc((strlen(ficheiro)+strlen(".palavras")+1)*sizeof(char));/* -- Insert code for memory allocation,  remember to add space for the dot (.), the filename extension and the string termination --*/ ;
  if ( nome == NULL ) {
    fprintf ( stderr, "ERROr: not enough memory available!\n" );
    exit ( 5 );
  }
